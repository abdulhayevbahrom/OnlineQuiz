...
const router = Router()
const { Admins, validateAdmin } = require('../models/adminModel')
...
router.post('/sign-in', async (req, res) => {
    try {
        ...
        const token = jwt.sign({ _id: exactAdmin._id, username, isActive: exactAdmin.isActive }, process.env.JWT_KEY)
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!deleteAdmin) {
            return res.status(400).json({ state: false, msg: "Admin o'chirilmadi", innerData: deleteAdmin })
        }
        res.status(200).json({ state: true, msg: "Admin o'chirildi", innerData: deleteAdmin })
    }
    catch {
        res.status(500).json({ state: false, msg: "Server error", innerData: null })
    }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ state: true, msg: "Admin updated", innerData: updatedAdmin })
    }
    catch {
